...
export default function CustomerDetailPage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(false);
    if (res.success) {
      router.push('/admin/super/customers');
    } else {
    ...
    return (
      <div className="max-w-4xl mx-auto p-12 text-center space-y-4">
        ...
        >
          ...
          <span>Back to Chain Contacts</span>
        </Link>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 border-b border-slate-100 pb-6">
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            <a
              href={`https://wa.me/91${customer.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Hello ${customer.name}, this is Devi Mobile (${customer.primaryStoreId}). Regarding your purchase and warranty support, we are always happy to assist you!`)}`}
              target="_blank"
              rel="noreferrer"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-1">
            ...
            <div className="text-xl sm:text-2xl font-black text-emerald-700">{formatINR(customer.totalSpent)}</div>
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-1">
            ...
            <div className="text-xl sm:text-2xl font-black text-slate-900">{customer.purchaseCount} Items</div>
              ...
              <div key={p.id} className="bg-white rounded-3xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                  ...
                  <div className="text-left sm:text-right">
                    <div className="text-lg font-black text-emerald-700">{formatINR(p.amount)}</div>
                    <span className="text-[10px] text-slate-400 block">
                      {new Date(p.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                </div>
              ...
              <div key={l.id} className="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs bg-slate-50 p-3 rounded-2xl border border-slate-100">
                  <div>
                    ...
                    <span className="font-bold text-emerald-700">{formatINR(l.budget)}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block font-bold uppercase">Logged At</span>
                    <span className="font-bold text-slate-800">{new Date(l.date).toLocaleDateString('en-IN')}</span>
                  </div>
                  <div>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Are you sure you want to permanently delete <strong className="text-slate-900">{customer.name}</strong> ({customer.phone})? You will be redirected back to the customers directory.
              </p>
            </div>
