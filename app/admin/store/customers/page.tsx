...
export default function StoreCustomersPage() {
  const [activeStoreId, setActiveStoreId] = useState<'DM-01' | 'DM-02'>('DM-01');
  ...
  useEffect(() => {
    let isMounted = true;
    async function load() {
      const list = await fetchCloudCustomers(activeStoreId);
  ...
  const filtered = customers
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'purchases') return (b.purchaseCount || 0) - (a.purchaseCount || 0);
  ...
  const totalPurchasesAll = (customers || []).reduce((sum, c) => sum + (c?.purchaseCount || 0), 0);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(false);
    if (res.success) {
      setDeleteSuccessToast(`Customer ${customerToDelete.name || ''} successfully deleted.`);
    ...
    <div className="max-w-7xl mx-auto space-y-6 font-sans pb-16">
      ...
      <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-black text-slate-900">Customer Contacts & CRM</h1>
          </div>
        </div>

        {/* Store Badge (No Switcher for Store Admin) */}
        <div className="self-start sm:self-auto">
          <span className="px-3.5 py-2 rounded-xl bg-slate-100 text-slate-800 text-xs font-bold border border-slate-200 block">
            Branch: {activeStoreId} ({activeStoreId === 'DM-02' ? 'Freeganj 2.0' : 'Kanthal Flagship'})
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-1">
          <div className="text-xs font-bold text-slate-400 uppercase">Registered Customers</div>
          <div className="text-2xl font-black text-slate-900">{customers.length} Buyers</div>
          <div className="text-[10px] text-slate-400">Branch {activeStoreId} Base</div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-1">
          <div className="text-xs font-bold text-slate-400 uppercase">Total Devices Billed</div>
          <div className="text-2xl font-black text-brand-600">{totalPurchasesAll} Units</div>
          <div className="text-[10px] text-slate-400">Lifetime purchases</div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-1 bg-gradient-to-br from-emerald-50/40 to-white">
          <div className="text-xs font-bold text-emerald-700 uppercase">Customer Lifetime Spend</div>
          <div className="text-2xl font-black text-emerald-700">{formatINR(totalSpentAll)}</div>
          <div className="text-[10px] text-emerald-600 font-semibold">Total revenue generated</div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-1 bg-gradient-to-br from-rose-50/40 to-white">
          ...
          <div className="text-[10px] text-rose-600 font-semibold">Store {activeStoreId} Unpaid</div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-1">
          <div className="text-xs font-bold text-slate-400 uppercase">Avg Spend / Buyer</div>
          <div className="text-2xl font-black text-slate-900">
            {formatINR(customers.length > 0 ? Math.round(totalSpentAll / customers.length) : 0)}
          </div>
          <div className="text-[10px] text-slate-400">Average transaction size</div>
        </div>
      </div>
      ...
      {/* Customer List */}
      <ErrorBoundary title="Customer Directory" description="Unable to render customer profiles due to a localized issue. Please retry.">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Jaise hi POS counter par koi sale complete hogi ya lead create hogi, customer profile yahan automatically save hokar dikhegi.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(customer => {
              ...
              const leadsList = customer?.leads || [];
              return (
                <div
                  key={customer.id}
                  className="bg-white rounded-3xl border border-slate-200 p-5 space-y-4 shadow-sm hover:shadow-md hover:border-brand-300 transition-all flex flex-col justify-between"
                >
                  <div className="space-y-3">
                    {/* Top Bar: Initial avatar & Phone */}
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <div className="w-10 h-10 rounded-2xl bg-brand-600 text-white flex items-center justify-center font-black text-base shadow-sm">
                      ...
                      <div className="flex items-center gap-1.5">
                        {(customer.totalDue || 0) > 0 && (
                          <span className="px-2 py-0.5 rounded-full bg-rose-100 text-rose-800 border border-rose-200 text-[10px] font-black animate-pulse">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs bg-slate-50 p-3 rounded-2xl border border-slate-100">
                      <div>
                        ...
                        <span className="font-black text-emerald-700 text-[11px]">{formatINR(customer.totalSpent || 0)}</span>
                      </div>
                      <div>
                        ...
                        <span className="font-bold text-slate-800 text-[11px] flex items-center gap-0.5">
                          <ShoppingBag className="w-3 h-3 text-slate-400" /> {customer.purchaseCount || 0}
                        </span>
                      </div>
                      <div>
                    ...
                    {purchasesList.length > 0 ? (
                      <div className="text-xs bg-blue-50/50 p-2.5 rounded-xl border border-blue-100 space-y-1">
                        ...
                        <div className="font-bold text-slate-900 flex items-center gap-1 truncate">
                          ...
                          <span className="truncate">{purchasesList[0]?.productName || 'Device'}</span>
                        </div>
                        <div className="text-[10px] text-slate-400 font-mono">
                          IMEI: {purchasesList[0]?.imei || 'N/A'}
                        </div>
                      </div>
                    ) : leadsList.length > 0 ? (
                      <div className="text-xs bg-amber-50/50 p-2.5 rounded-xl border border-amber-100 space-y-1">
                        ...
                        <div className="font-bold text-slate-900 truncate">
                          Interested in: {leadsList[0]?.model || 'Device'}
                        </div>
                        <div className="text-[10px] text-slate-400">
                          Budget: {formatINR(leadsList[0]?.budget || 0)} • {leadsList[0]?.status || ''}
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100">
                    <div className="flex items-center gap-2">
                      <a
                        href={`https://wa.me/91${cleanPhone}?text=${encodeURIComponent(`Hello ${customer.name || 'Customer'}, greeting from Devi Mobile (${activeStoreId}). We have special offers and service support available for you!`)}`}
                        target="_blank"
                        rel="noreferrer"
                      ...
                      <Link
                        ...
                        className="py-2 px-3 rounded-xl bg-brand-50 hover:bg-brand-100 text-brand-700 font-bold text-xs flex items-center justify-center gap-1 transition-colors min-h-[44px]"
                      >
                        <span>Profile</span>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Are you sure you want to permanently delete <strong className="text-slate-900">{customerToDelete.name}</strong> ({customerToDelete.phone})? This will remove their record from the branch customer directory.
              </p>
            </div>
